...
contract HATTimelockController is TimelockController {
    ...
    function setVaultVisibility(HATVault _vault, bool _visible) external onlyRole(MANAGER_ROLE) {
        _vault.registry().setVaultVisibility(address(_vault), _visible);
    }

    function setVaultDescription(HATVault _vault, string memory _descriptionHash) external onlyRole(MANAGER_ROLE) {
